...
```
La distribution de la variable First Time Asylum Applicants (premières demandes d’asile) montre une forte asymétrie, avec plusieurs valeurs extrêmes très élevées. Ces valeurs, représentées par des points au-dessus des moustaches, indiquent que quelques pays reçoivent un nombre disproportionné de premières demandes d’asile par rapport à la majorité. La médiane basse reflète que la moitié des pays ont une valeur faible pour la demande d’asile qui est en dessous de cette valeur médiane .
...
La distribution de la variable Minimum Wage (salaire minimum) montre une dispersion modérée avec quelques valeurs basses qui se démarquent, visibles sous forme de moustaches allongées. La médiane, située dans la moitié inférieure de la boîte, indique que la moitié des pays ont un salaire minimum inférieur à cette valeur médiane.

La variable People at Risk of Poverty or Exclusion (personnes à risque de pauvreté ou d’exclusion sociale) présente une distribution relativement homogène. La médiane, légèrement orientée vers le bas de la boîte . La taille modérée de la boîte et l’absence de valeurs aberrantes suggèrent que les pays étudiés affichent des niveaux de risque relativement similaires pour cet indicateur.
```{r}
...
La variable montre également deux valeurs aberrantes pour des pays ayant une part particulièrement élevée d'énergies renouvelables. La médiane, située vers le bas de la boîte, indique que la moitié des pays a des parts d'énergies renouvelables faibles ou proches de cette valeur, tandis que l'autre moitié des pays a des parts plus élevées.
...
plot(res_acp$x[, 1], res_acp$x[, 2], 
     ...
     pch = 20, 
     xlab = "PC1", ylab = "PC2",
     main = "Plan factoriel  classes CAH")
...
plot(res_acp$x[, 1], res_acp$x[, 2], 
     ...
     pch = 20, 
     xlab = "PC1", ylab = "PC2",
     main = "Plan factoriel  classes k-means")
...
silhouette_cah <- silhouette(classes_cah, dist(euro_data_normalized))
cat("Silhouette moyenne pour CAH :", mean(silhouette_cah[, 3]), "\n")

# Calcul de l'indice de silhouette pour k-means
silhouette_kmeans <- silhouette(classes_kmeans, dist(euro_data_normalized))
cat("Silhouette moyenne pour k-means :", mean(silhouette_kmeans[, 3]), "\n")
...
Les résultats des classifications obtenues par CAH et par k-means ont été projetés sur le plan formé par les deux premières composantes principales. Concernant les classes obtenues par CAH, les regroupements semblent compacts visuellement, mais on observe un certain chevauchement entre certaines observations. Les classes obtenues par k-means, quant à elles, paraissent légèrement plus homogènes et distinctes.

Pour objectiver cette observation visuelle, nous avons calculé l’indice de silhouette pour chaque méthode :

    Indice de silhouette moyen pour CAH : 0.122
    Indice de silhouette moyen pour k-means : 0.139

L’indice de silhouette, qui mesure à la fois la compacité des classes et leur séparation, est légèrement supérieur pour k-means, confirmant que cette méthode offre des regroupements plus cohérents et mieux séparés que CAH dans ce contexte.

La classification par k-means optimise directement la compacité des groupes, ce qui est visible dans leur projection et confirmé par l’indice de silhouette. Cependant, elle reste sensible à l’initialisation aléatoire des centres, ce qui peut influencer les résultats. De son côté, la méthode CAH, qui repose sur des fusions hiérarchiques successives, est moins sensible à l’initialisation, mais peut produire des regroupements légèrement moins homogènes en raison de sa construction basée uniquement sur les distances entre observations.

En conclusion, l’ACP combinée aux représentations graphiques et à l’analyse quantitative permet de comparer les deux méthodes de classification. Bien que les deux méthodes soient globalement cohérentes, k-means semble offrir de meilleurs résultats en termes d’homogénéité et de séparation des classes, comme l’indique l’indice de silhouette. Cependant, le choix de la méthode dépend des objectifs de l’étude : k-means convient mieux pour obtenir des groupes compacts et homogènes, tandis que CAH peut être plus adapté lorsqu’une hiérarchie des regroupements est recherchée.
...
```{r}
# Étape 1 : Créer le data frame avec les coordonnées factorielles
coord_acp <- data.frame(PC1 = res_acp$x[, 1], 
                        PC2 = res_acp$x[, 2], 
                        Classe_CAH = classes_cah)

# Étape 2 : Sélectionner la zone restreinte
zone_restante <- coord_acp[coord_acp$PC1 > -1 & coord_acp$PC1 < 1 & 
                           coord_acp$PC2 > -1 & coord_acp$PC2 < 1, ]

# Étape 3 : Appliquer l’algorithme k-means uniquement sur la zone restreinte
set.seed(42)  # Fixer la graine pour reproductibilité
kmeans_zone <- kmeans(zone_restante[, c("PC1", "PC2")], centers = 4, nstart = 10)

# Ajouter les classes k-means obtenues à la zone restreinte
zone_restante$Classe_kmeans <- kmeans_zone$cluster

# Afficher les pays dans la zone restreinte avec leurs classes
cat("Pays dans la zone sélectionnée avec classes CAH et k-means :\n")
print(zone_restante)

# Étape 4 : Représenter graphiquement la zone avec classes CAH
par(mfrow = c(1, 1))  # Deux graphiques côte à côte

# Graphique avec classes CAH
...
# Graphique avec classes k-means
plot(zone_restante$PC1, zone_restante$PC2, 
     col = zone_restante$Classe_kmeans, 
     pch = 20, 
     xlab = "PC1", ylab = "PC2",
     main = "Zone restreinte avec classes k-means")
text(zone_restante$PC1, zone_restante$PC2, 
     labels = rownames(zone_restante), pos = 4, cex = 0.8)
...
Les pays identifiés dans cette zone restreinte sont : 1 (Autriche), 9 (Finlande), 19 (Malte), 21 (Pologne), 25 (Slovénie) et 27 (Suède). Selon les classes issues des deux méthodes de classification (CAH et k-means) :
...
Classe k-means : Contrairement à CAH, les pays sont répartis dans différentes classes (Classe 1, Classe 2, Classe 3 et Classe 4). Cela montre une divergence dans la manière dont k-means interprète les proximités, possiblement influencée par la minimisation de l'inertie intra-classe.

Dans le graphique, on observe que les pays 21 (Pologne) et 27 (Suède) sont proches sur le plan factoriel (PC1 et PC2), suggérant une similarité forte entre eux. Les pays 19 (Malte), 21 (Pologne), 27 (Suède) et 25 (Slovénie) forment un groupe légèrement dispersé mais globalement cohérent, indiquant des caractéristiques modérément similaires. Les pays 1 (Autriche) et 9 (Finlande) sont éloignés du reste du groupe, bien qu'ils fassent partie de la même classe selon CAH, ce qui pourrait indiquer une particularité spécifique sur certaines variables.

Cette analyse montre que, bien que les deux méthodes de classification (CAH et k-means) regroupent les pays de manière globalement similaire, certaines différences subsistent, notamment dans la répartition des classes par k-means, où les proximités peuvent être interprétées différemment en raison de la méthodologie utilisée.
