...
```{r}
# Étape 1 : Créer un data frame avec les coordonnées factorielles et les classes CAH
coord_acp <- data.frame(PC1 = res_acp$x[, 1], 
                        PC2 = res_acp$x[, 2], 
                        Classe_CAH = classes_cah)

# Étape 2 : Sélectionner une zone restreinte (PC1 et PC2 dans l'intervalle [-1, 1])
zone_restante <- coord_acp[coord_acp$PC1 > -1 & coord_acp$PC1 < 1 & 
                           coord_acp$PC2 > -1 & coord_acp$PC2 < 1, ]

# Étape 3 : Afficher les pays sélectionnés
cat("Pays dans la zone restreinte :\n")
print(zone_restante)

# Étape 4 : Représenter graphiquement la zone restreinte avec les classes CAH
...
# Ajouter les noms des pays sur le graphique
...
Classe k-means : Contrairement à CAH, les pays sont répartis dans différentes classes (1, 2, 3, et  4 ). Cela montre une divergence dans la manière dont k-means interprète les proximités, possiblement influencée par la minimisation de l'inertie intra-classe.

Analyse des proximités

Dans le graphique, on observe que :

  Les pays 21 et 27 sont proches sur le plan factoriel (PC1 et PC2), suggérant une similarité forte entre eux.
  Les pays 19, 21, 27  et 25 forment un groupe légèrement dispersé mais globalement cohérent, indiquant des caractéristiques modérément similaires.
  Les pays 1 et 9  sont  éloignés du reste du groupe, bien qu'ils fassent  partie de la même classe selon CAH, ce qui pourrait indiquer une particularité spécifique sur certaines variables.
